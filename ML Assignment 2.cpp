...
vector<double> findCoef(vector<vector<double>> train_data) {
	...
	for (int x = 0; x < train_data[0].size(); x++) {
		data_matrix[0][x] = 1;
		data_matrix[1][x] = train_data[1][x];
	}
...
vector<double> predict(vector<vector<double>> testMatrix, vector<double> weights)
{
	...
	{
		prob[x] = exp(pred[x]) / (1 + exp(pred[x]));
	}
...
int main(int argc, char** argv)
{
	...
	{
		if (preds[x] == 1 && test_data[0][x] == 1)
			tP++;
		if (preds[x] == 1 && test_data[0][x] == 0)
			fP++;
		if (preds[x] == 0 && test_data[0][x] == 1)
			fN++;
		if (preds[x] == 0 && test_data[0][x] == 0)
			tN++;
	}
